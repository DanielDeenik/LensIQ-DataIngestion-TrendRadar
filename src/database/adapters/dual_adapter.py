...
This module provides a dual database adapter that writes to multiple 
MongoDB instances.
"""
...
class DualDatabaseAdapter(DatabaseAdapter):
    """Dual database adapter that writes to multiple MongoDB instances."""

    def __init__(self, primary_adapter_type: str = 'mongodb', 
                 secondary_adapter_type: str = 'mongodb'):
        """
        Initialize the dual database adapter.

        Args:
            primary_adapter_type: Primary database adapter type ('mongodb')
            secondary_adapter_type: Secondary database adapter type ('mongodb')
        """
        # Initialize adapters (both MongoDB for now)
    ...
    def connect(self) -> bool:
        ...
        Returns:
            bool: True if connection to primary database successful, 
                  False otherwise
        """
        # Connect to primary database
        self.primary_connected = self.primary_adapter.connect()
        if not self.primary_connected:
            logger.error("Failed to connect to primary database (MongoDB)")
            return False

        # Connect to secondary database
        self.secondary_connected = self.secondary_adapter.connect()
        if not self.secondary_connected:
            logger.warning("Failed to connect to secondary database (MongoDB)")
            # Continue even if secondary connection fails

        logger.info("Connected to primary database (MongoDB)")
        if self.secondary_connected:
            logger.info("Connected to secondary database (MongoDB)")
    ...
    def find_one(self, collection_name: str, query: Dict = None, 
        ...
        if self.primary_connected:
            return self.primary_adapter.find_one(collection_name, query, 
                                                projection)
        return None
    ...
    def find(self, collection_name: str, query: Dict = None, 
        ...
        # Read from primary database
        if self.primary_connected:
            return self.primary_adapter.find(collection_name, query, 
    ...
    def insert_one(self, collection_name: str, 
        ...
        Returns:
            Inserted document ID from primary database or None if error
        """
        primary_id = None

        # Insert into primary database
        if self.primary_connected:
            primary_id = self.primary_adapter.insert_one(collection_name, 
                                                        document)
            if not primary_id:
                logger.error("Failed to insert document into primary "
                           "database (MongoDB)")
                return None

        # Insert into secondary database
        if self.secondary_connected:
            # Ensure document has the same ID for consistency
            if primary_id and '_id' not in document:
                document['_id'] = primary_id

            secondary_id = self.secondary_adapter.insert_one(collection_name, 
                                                           document)
            if not secondary_id:
                logger.warning("Failed to insert document into secondary "
                             "database (MongoDB)")
    ...
    def insert_many(self, collection_name: str, 
        ...
        Returns:
            List of inserted document IDs from primary database or 
            empty list if error
        """
        primary_ids = []
        ...
        # Insert into primary database
        if self.primary_connected:
            primary_ids = self.primary_adapter.insert_many(collection_name, 
                                                          documents)
            if not primary_ids:
                logger.error("Failed to insert documents into primary "
                           "database (MongoDB)")
                return []

        # Insert into secondary database
        if self.secondary_connected:
            # Ensure documents have the same IDs for consistency
            if primary_ids:
                for i, doc in enumerate(documents):
                    if i < len(primary_ids) and '_id' not in doc:
                        doc['_id'] = primary_ids[i]

            secondary_ids = self.secondary_adapter.insert_many(
                collection_name, documents)
            if not secondary_ids:
                logger.warning("Failed to insert documents into secondary "
                             "database (MongoDB)")
                # Continue even if secondary insert fails
    ...
    def update_one(self, collection_name: str, query: Dict, update: Dict, 
        ...
        if self.primary_connected:
            primary_result = self.primary_adapter.update_one(
                collection_name, query, update, upsert)
            if not primary_result:
                logger.error("Failed to update document in primary "
                           "database (MongoDB)")
                return False

        # Update in secondary database
        if self.secondary_connected:
            secondary_result = self.secondary_adapter.update_one(
                collection_name, query, update, upsert)
            if not secondary_result:
                logger.warning("Failed to update document in secondary "
                             "database (MongoDB)")
    ...
    def update_many(self, collection_name: str, query: Dict, update: Dict, 
        ...
        Returns:
            Number of documents updated in primary database
        """
        primary_count = 0

        # Update in primary database
        if self.primary_connected:
            primary_count = self.primary_adapter.update_many(
                collection_name, query, update, upsert)
            if primary_count == 0:
                logger.warning("No documents updated in primary "
                             "database (MongoDB)")

        # Update in secondary database
        if self.secondary_connected:
            secondary_count = self.secondary_adapter.update_many(
                collection_name, query, update, upsert)
            if secondary_count == 0:
                logger.warning("No documents updated in secondary "
                             "database (MongoDB)")
                # Continue even if secondary update fails
    ...
    def delete_one(self, collection_name: str, query: Dict) -> bool:
        ...
        if self.primary_connected:
            primary_result = self.primary_adapter.delete_one(collection_name, 
                                                           query)
            if not primary_result:
                logger.error("Failed to delete document from primary "
                           "database (MongoDB)")
                return False

        # Delete from secondary database
        if self.secondary_connected:
            secondary_result = self.secondary_adapter.delete_one(
                collection_name, query)
            if not secondary_result:
                logger.warning("Failed to delete document from secondary "
                             "database (MongoDB)")
    ...
    def delete_many(self, collection_name: str, query: Dict) -> int:
        ...
        # Delete from primary database
        if self.primary_connected:
            primary_count = self.primary_adapter.delete_many(collection_name, 
                                                           query)
            if primary_count == 0:
                logger.warning("No documents deleted from primary "
                             "database (MongoDB)")

        # Delete from secondary database
        if self.secondary_connected:
            secondary_count = self.secondary_adapter.delete_many(
                collection_name, query)
            if secondary_count == 0:
                logger.warning("No documents deleted from secondary "
                             "database (MongoDB)")
                # Continue even if secondary delete fails
    ...
    def count_documents(self, collection_name: str, 
                       query: Dict = None) -> int:
        """
        Count documents in a collection from primary database.
        ...
        """
        if self.primary_connected:
            return self.primary_adapter.count_documents(collection_name, 
                                                       query)
        return 0
    ...
    def create_story(self, story_data: Dict) -> Optional[str]:
        """
        Create a story in both databases.

        Args:
            story_data: Story data to insert

        Returns:
            Story ID from primary database or None if error
        """
        primary_id = None
        ...
        if self.primary_connected:
            ...
            if not primary_id:
                logger.error("Failed to create story in primary "
                           "database (MongoDB)")
                return None

        # Insert into secondary database
        if self.secondary_connected:
            # Ensure story has the same ID for consistency
            ...
            if not secondary_id:
                logger.warning("Failed to create story in secondary "
                             "database (MongoDB)")
                # Continue even if secondary insert fails
